...
def embed(img_path, data_path, img_hz="png"):
    ...
    cv2.imwrite(f'../img/steg/LV/{img_name}_LV_steg.{img_hz}', embedded_image)
    print(f"数据嵌入成功。嵌入数据的图像已保存为'img/steg/LV/{img_name}_LV_steg.{img_hz}'。")
    # 单独保存掩码图像
    cv2.imwrite(f'../img/mask/{img_name}_LV_mask.{img_hz}', mask)
    print(f"嵌入掩码图像已保存为'img/mask/{img_name}_LV_mask.{img_hz}'。")
